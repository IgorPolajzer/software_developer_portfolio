    ...
    <div className="flex flex-col items-center w-full p-20">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 w-full">
        {skills.map((skill, idx) => (
          <div
            key={idx}
            className="flex flex-col items-center justify-center p-4 bg-white/10 rounded-lg shadow hover:scale-105 transform transition border-2 border-[var(--color-text-base)]"
          >
            <img src={skill.iconPath} alt={skill.label} className="w-12 h-12 mb-2" />
            <p className="text-text-[var(--color-primary)] font-semibold text-center text-sm">{skill.label}</p>
          </div>
        ))}
      </div>
    </div>
